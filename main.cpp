...
// Luis Martin 90.951.273
// Luis Ochoa 28.310.422
// Franklin Quintana 
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
void darfecha(dir **p)
{
	...
	if(((g->tm_mday)>=1)&&((g->tm_mday)<=9)){
		sprintf(day, "0%d", g->tm_mday);
	}else{
		sprintf(day, "%d", g->tm_mday);
	}
	if(((g->tm_mon+1)>=1)&&((g->tm_mon+1)<=9)){
		sprintf(month, "0%d", g->tm_mon + 1);
	}else{
...
void sobreescribir(dir **p, dir **ax, dir **bx, int x, int op)
{
	...
	if (op == 0)
	{
		while (1)
		{
			printf("%cExiste un directorio de igual nombre en la ubicacion desea continuar? (S,N): ", 168);
			...
			if (validar(1, opc))
			{
				...
				case 'S':
					while (1)
					{
						printf("%cDesea sobreescribir (S) o unir (U)?: ", 168);
						scanf("%s", &opc[0]);
						fflush(stdin);
						if (validar(1, opc))
						{
		...
		if (!comprobar((*p)->pfa))
		{
			...
			if (g->pfa)
			{
				if(g->ppa->ppa) {
					...
					g=g->pfa;mclovin(g->pfa, (*p)->pfa, *p, g);
					g=g->ppa;
					borrartodo(g->pfa);
					darfecha(p);
					delete (g);
...
void SHD(dir *q, dir **ax, char **ordenado)
{
	...
	if ((*ax) && ((*ax)->ppa) && ((*ax)->pfa))
	{
		...
		else
		{
			if ((ordenado[1]) && (ordenado[2]))
			{
				if ((!stricmp(ordenado[2], "/s")) || (!stricmp(ordenado[2], "/h")))
				{
					printf("\n");
					printv2(*ax);
					printf("\n");
	...
	else
	{
		 if(!(*ax)->pfa)
            if(ordenado[1]){
				if ((stricmp(ordenado[1], "/s")) && (stricmp(ordenado[1], "/h"))) mostrar(q, ax, ordenado[1], s, h);
				else{
					printf("\n");
					printv2(*ax);
					printf("   \n\n");
...
int verificartokenv2(char *token) {
	char x[1024], *t;
	strcpy(x, token);
	int resp=0;
	for (int unsigned i=0;i<strlen(x);i++){
...
void mdd(char *ordenado1, char *ordenado2, char *ordenado3, char *ordenado4, dir *q, dir *ax)
{
	...
	if ((ordenado1 && ordenado2) || (ordenado1 && ordenado2 && ordenado3) || (ordenado1 && ordenado2 && ordenado3 && ordenado4))
	{
		...
		if(auxRoot){
			...
			if(valor1) {
				if(strlen(valor1)>0) {	
					if(!stricmp(param1,"/r")||!stricmp(param1,"/h")) 
						if(strlen(valor1)!=1){
							printf("ERROR: Comando invalido\n");return;
						}
				} else {
			...
			if (ordenado3)
			{
				...
				if(valor2) {
					if(strlen(valor2)>0) {
						if(!stricmp(param2,"/r")||!stricmp(param2,"/h")) 
							if(strlen(valor2)!=1) {
								printf("ERROR: Comando invalido\n");return;
							}
					} else {
			...
			if (ordenado4)
			{
				...
				if(valor3) {
					if(strlen(valor3)>0) {
						if(!stricmp(param3,"/r")||!stricmp(param3,"/h")) 
							if(strlen(valor3)!=1) {
								printf("ERROR: Comando invalido\n");return;
							}
					} else {
